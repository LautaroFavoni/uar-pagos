...
export default function DesignacionesPage() {
  ...
  const handleCopyMatchWhatsApp = (match: any) => {
    let text = `🏉 *DESIGNACIÓN ${BRAND.nombre}*\n`
    text += `━━━━━━━━━━━━━━\n`
  ...
  const copyWhatsAppText = () => {
    let text = `⚽ *DESIGNACIONES ${BRAND.nombre} - SEMANA ${semana}* ⚽\n\n`
    ...
    text += `_Generado por ${BRAND.nombre} Pagos_`
    navigator.clipboard.writeText(text)
    ...
    <div className={cn("space-y-8 pb-20", isPhotoMode && "fixed inset-0 z-[100] bg-white p-10 overflow-auto md:ml-0")}>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-1">
                <h2 className="text-3xl font-black tracking-tight text-brand">Designaciones</h2>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex gap-2">
                ...
                <Button 
                    className="bg-brand hover:bg-brand-hover text-white gap-2 font-bold h-11 px-6 rounded-xl"
      ...
      {isPhotoMode && (
        <div className="flex items-center justify-between mb-8 pb-4 border-b">
           <div className="flex items-center gap-4">
              <span className="bg-brand text-white p-2 rounded-lg font-black italic">{BRAND.nombre}</span>
              <div>
        ...
        {diasUnicos.length > 0 ? diasUnicos.map(fecha => (
            <div key={fecha} className="space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div className="bg-zinc-100 px-6 py-2 rounded-full border text-brand font-black italic text-xs uppercase tracking-widest flex items-center gap-2">
                            ...
                            <div className="bg-white p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                <div className="flex items-center gap-4 flex-1">
                                    <div className="flex flex-col items-center justify-center bg-zinc-900 text-white min-w-[70px] h-[70px] rounded-xl overflow-hidden">
                                        ...
                                        <span className="text-[8px] font-black opacity-40 italic">{BRAND.nombre}-MATCH</span>
                                    </div>
                                ...
                                {!isPhotoMode && (
                                    <div className="flex items-center gap-2">
                                        <Button 
                                            variant="outline" 
                                            size="sm" 
                                            className="rounded-full gap-2 font-black text-[10px] uppercase border-brand text-brand hover:bg-brand hover:text-white"
                                        ...
                                        <Button 
                                            variant="ghost" 
                                            size="icon" 
                                            className="h-8 w-8 rounded-full text-zinc-400 hover:text-brand hover:bg-blue-50"
      ...
      {isPhotoMode && (
         <div className="mt-10 text-center text-zinc-300 font-bold text-sm tracking-widest pt-10 border-t uppercase">
            Sistema Oficial de Gestión {BRAND.nombre} — {anio}
         </div>
      )}
